...
def home(request):
    ...
    today_update = models.Update.objects.filter(date__year = year, date__month = month, date__day = this_day).order_by('-date')[0:1]
...
# My function that generates the pdf if not found and update on my current pdf if found, this function generates the pdf monthely...
...
def view_final_users_report_pdf(request, slug_code):
    # report = get_object_or_404(models.AllBillsPDF, id=report_id)
    report = models.AllBillsPDF.objects.get(slug_code = slug_code)
    report_path = report.pdf_file.path
    ...
    response['Content-Disposition'] = 'inline; filename="{}"'.format(os.path.basename(report_path))
...
def download_final_users_report_pdf(request, slug_code):
    report = models.AllBillsPDF.objects.get(slug_code=slug_code)
...
def chart_view(request):
    ...
    # Calling the two function that extract and render the data from my final_user_report.html to a pdf file...
